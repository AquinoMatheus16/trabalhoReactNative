...
public class ProdutoController {
	...
	@ApiResponses(value = { @ApiResponse(code = 201, message = "Produto adcionado"),
			...
			@ApiResponse(code = 403, message = "Não há permissão para acessar o recurso"),
			@ApiResponse(code = 505, message = "Exceção interna da aplicação"), })
	public ResponseEntity<ImagemProdutoDTO> cadastrar(@Valid @RequestPart ProdutoDTO2 produto,
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Produto Atualizado"),
			...
			@ApiResponse(code = 404, message = "Recurso não encontrado"),
			@ApiResponse(code = 505, message = "Exceção interna da aplicação"), })
	public ResponseEntity<ImagemProdutoDTO> salvar(@PathVariable Long idProduto,
			@Valid @RequestPart ProdutoDTO2 produto, @RequestPart MultipartFile file) throws IOException {
